...
================================================================================
    [+] Developer   : Akbarion
    [+] Version     : DDS FREE EDITION (v5.0 - UNTOUCHABLE EDITION)
    [+] UI Theme    : Sodium Menu / MySuperHub
    [+] Features    : RNG Movement, Anti-Cepu, Panic Alert, FPS Boost, Clean Code
================================================================================
]]--
...
-- ============================================================================
-- // 3. HUMANIZATION SYSTEM (RNG) & ANTI-CEPU
-- ============================================================================
-- Jeda waktu acak persis kayak manusia asli
local function rWait(minSec, maxSec)
    local min = minSec or 0.5
    local max = maxSec or 1.5
    task.wait(math.random(min * 1000, max * 1000) / 1000)
end

-- Radar Anti-Cepu: Berhenti kalau ada player deket
local function IsPlayerNearby(radius)
    if not State.AntiCepu then return false end
    
    for _, p in ipairs(Services.Players:GetPlayers()) do
        if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
            local dist = (p.Character.HumanoidRootPart.Position - CharRef.Root.Position).Magnitude
            if dist <= (radius or 15) then
                return true
            end
        end
    end
...
-- ============================================================================
-- // 4. WEBHOOK SYSTEM (MONITORING & PANIC ALERT)
-- ============================================================================
...
local function SendDiscordReport()
    if not State.WebhookEnabled or State.WebhookURL == "" or State.WebhookURL == "Paste Link Disini" then 
        return 
    end
    ...
    pcall(function()
        ...
        if request then
            ...
            local data = {
                ["embeds"] = {{
                    ["title"] = "⚙️ King Akbarion V5.0 - Monitoring",
...
-- ============================================================================
-- // 5. ALARM TRIGGERS (ADMIN & ERROR SENSOR)
-- ============================================================================
...
do
    ...
    local statLbl = createLabel("Memuat Modul Anti-Cepu...", 200, 12)
    ...
    local function tween(obj, props, t)
        local tweenInfo = TweenInfo.new(t, Enum.EasingStyle.Quart, Enum.EasingDirection.Out)
        Services.TweenService:Create(obj, tweenInfo, props):Play()
    end

    local loadSteps = {
        { "Menyiapkan RNG Humanize...",    0.30 },
...
local function ToggleBlackScreen(state)
    ...
    if state then
        if not BlackScreenGui then
            ...
            txt.Text = "🌑 MODE HEMAT BATERAI AKTIF 🌑\nKing Akbarion V5 Sedang Mencari Cuan..."
...
-- ============================================================================
-- // 9. UTILITY METHODS (RNG PATHING + SENSORS)
-- ============================================================================
local function WalkToPoint(targetPos)
    ...
    -- [RNG PATHING] Bot tidak nginjek garis yang sama persis
    local randomOffsetX = math.random(-15, 15) / 10 -- Goyang -1.5 s/d +1.5 stud
    local randomOffsetZ = math.random(-15, 15) / 10
    local humanizedPos = targetPos + Vector3.new(randomOffsetX, 0, randomOffsetZ)
    
    CharRef.Humanoid:MoveTo(humanizedPos)
    
    local timeOut = 10 
    while timeOut > 0 and State.IsFarming do
        local flatDist = Vector3.new(CharRef.Root.Position.X, 0, CharRef.Root.Position.Z) - Vector3.new(humanizedPos.X, 0, humanizedPos.Z)
        if flatDist.Magnitude < 3 then 
            break 
        end
        task.wait(0.1)
        timeOut -= 0.1
...
local function DoHold(prompt)
    ...
    pcall(function()
        ...
        rWait((prompt.HoldDuration or 1) + 0.2, (prompt.HoldDuration or 1) + 0.6) -- RNG Hold Delay
        prompt:InputHoldEnd()
    end)
    rWait(0.1, 0.3)
    return true
...
-- ============================================================================
-- // 10. AI MINIGAME (RNG CLICK SENSOR)
-- ============================================================================
local function StartMinigameAI()
    ...
    State.AiThread = task.spawn(function()
        ...
        while State.IsFarming do
            ...
            -- [RNG CLICKING] Titik sentuh layar meleset dikit-dikit kayak manusia
            ...
            for _, v in pairs(mf:GetDescendants()) do
                if v:IsA("Frame") or v:IsA("ImageLabel") then
                    ...
                    if nm:find("pill") or nm:find("indicator") or nm:find("player") or nm:find("handle") then 
                        pill = v 
                    end
                    if nm:find("target") or nm:find("zone") or nm:find("goal") or nm:find("safe") then 
                        bar = v 
                    end
                end
            end
            ...
            if not pill then 
                pill = FindByColor(mf, Constants.COLOR_ORANGE, 0.6) 
            end
            if not bar then 
                bar = FindByColor(mf, Constants.COLOR_GREEN, 0.6) 
            end
            ...
            if not pill or not bar then
                ...
                table.sort(allEls, function(a, b) 
...
-- ============================================================================
-- // 11. MAIN FARMING LOOP (HUMANIZED & ANTI-CEPU)
-- ============================================================================
...
local function FarmLoop()
    ...
    while State.IsFarming do
        ...
        -- ==========================================
        -- [ANTI-CEPU SYSTEM] Cek Player Dekat
        -- ==========================================
        while IsPlayerNearby(15) and State.IsFarming do
            State.StatusText = "👀 Ada Player Deket! Pura-pura AFK..."
            rWait(1.5, 3.0) -- Diam layaknya manusia yang lagi AFK/Scroll TikTok
        end
        ...
        while not HasPendingOrder() and not IsMachineBroken() and State.IsFarming do
            ...
            -- Pengecekan ekstra kalau tiba-tiba ada player deket saat lagi nunggu
            if IsPlayerNearby(15) then 
                break 
            end 
        end
        
        if not State.IsFarming or IsPlayerNearby(15) then 
            continue 
        end
        ...
        if HasPendingOrder() then
            ...
            if brewPrompt then
                ...
                while State.IsFarming do
                    ...
                    if not mf or not mf.Visible then 
                        break 
                    end
                    task.wait(0.5)
                end
            ...
            -- [RNG DELAY] Jeda istirahat acak layaknya manusia yang capek
            local humanDelay = State.ActionDelay + (math.random(-5, 10) / 10)
            rWait(humanDelay, humanDelay + 0.5)
        end
    end
end
...
-- ============================================================================
-- // 12. UI CONSTRUCTION (EXPANDED & CLEAN)
-- ============================================================================
...
local TabBarista = Window:Tab({ 
    Title = "Menu Utama", 
    Icon = "coffee", 
    Border = true 
})

local StatusParagraph = TabBarista:Paragraph({ 
    Title = "Info System", 
    Desc = "Lagi Santai Coy", 
    Transparent = true 
})

local OrderParagraph = TabBarista:Paragraph({ 
    Title = "Statistik", 
    Desc = "0", 
    Transparent = true 
})
...
TabBarista:Toggle({
    Title = "Gas Auto Barista (Humanized)",
    Desc = "Nyalain fitur auto nyeduh dengan pergerakan siluman",
    Icon = "play",
    Value = false,
...
TabWeb:Toggle({ 
    Title = "Nyalain Webhook (Laporan & Alarm)", 
    Callback = function(s) State.WebhookEnabled = s end 
})

TabWeb:Input({ 
    Title = "Link Webhook Discord", 
    Placeholder = "Paste Link Disini", 
    Callback = function(v) State.WebhookURL = v end 
})

TabWeb:Slider({ 
    Title = "Jeda Laporan (Menit)", 
    Value = {Min = 1, Max = 60, Default = 10}, 
    Callback = function(v) State.WebhookInterval = v end 
})

TabWeb:Section({ Title = "Test Fitur Keamanan" })

TabWeb:Button({ 
    Title = "Test Laporan Cuan", 
    Callback = function() 
...
TabWeb:Button({ 
    Title = "Test Alarm Darurat", 
    Callback = function() 
        State.WebhookEnabled = true
        SendPanicAlert("INI CUMA TEST ALARM BOSKU! 🚨")
...
TabBarista:Button({ 
    Title = "Ambil Job Manual", 
    Callback = function() task.spawn(TakeJob) end 
})

TabBarista:Button({ 
    Title = "Jalan ke Mesin Kopi", 
    Callback = function() FollowPath(Paths.START_TO_MACHINE) end 
})

TabBarista:Button({ 
    Title = "Jalan ke Kasir", 
    Callback = function() FollowPath(Paths.MACHINE_TO_CASHIER) end 
})

local TabSec = Window:Tab({ 
...
TabSec:Toggle({ 
    Title = "Radar Anti-Cepu (Proximity)", 
    Desc = "Bot pura-pura AFK jika ada player mendekat dalam radius 15 meter.", 
    Value = true, 
    Callback = function(state) State.AntiCepu = state end 
})

TabSec:Toggle({ 
    Title = "Radar Anti-Admin", 
    Icon = "user-minus", 
    Value = true, 
    Callback = function(state) State.AntiAdmin = state end 
})
...
TabSec:Button({ 
    Title = "Aktifkan Mode Kentang (FPS Boost)", 
    Desc = "Hapus semua tekstur dan bayangan biar enteng", 
    Callback = function() 
        WindUI:Notify({Title = "Anti-Lag Aktif", Content = "Lagi nyapu tekstur, awas ngelag dikit..."})
        task.spawn(OptimizeFPS) 
    end 
})

TabSec:Toggle({ 
    Title = "Mode Layar Hitam (Hemat Baterai)", 
    Desc = "Mematikan grafik 3D biar HP/PC gak panas pas AFK semalaman", 
    Value = false, 
    Callback = function(state) ToggleBlackScreen(state) end 
})
...
TabSettings:Slider({ 
    Title = "Jeda Dasar (Detik)", 
    Desc = "Akan otomatis diacak (RNG) oleh sistem agar lebih natural",
    Step = 1, 
...
WindUI:Notify({ 
    Title = "👑 KING AKBARION V5.0 READY!", 
    Content = "Sistem Anti-Cepu & RNG Humanize udah aktif. Siluman cuy!", 
    Duration = 5 
})
